...
void GLCD_LcdInit(void)	{
    ...
    LcdInstructionWrite(LCD_DISP_OFF);          // 0xAE
    LcdInstructionWrite(LCD_SET_RATIO_OSC);     // 0xD5
...
//set at main LcdInstructionWrite(LCD_SET_SCAN_NOR);
    LcdInstructionWrite(LCD_SET_COL_ADDR);      // 0x21
    LcdInstructionWrite(0);                     // Start
    LcdInstructionWrite(127);                   // End
	LcdInstructionWrite(LCD_SET_AREA_COLOR);    // 0xD8
...
//reset value    LcdInstructionWrite(0x12);
    ...
    LcdInstructionWrite(LCD_MEM_ADDRESSING);    // 0x20
    LcdInstructionWrite(0x00);                  // Horizontal Addressing mode
    LcdInstructionWrite(LCD_EON_OFF);           // 0xA4
...
void dma_display(void) {
    ...
    DMA.CH2.TRFCNT    = DISPLAY_DATA_SIZE;       // buffer size
